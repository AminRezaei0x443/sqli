...
class CreateQueryTree(Transformer):
    ...
    def column_eq(self, column_i, column_j):
        return SimplePredicate(
            SimplePredicateType.ColumnToColumn, column_i, column_j,
        )

    @v_args(inline=True)
    def column_cond(self, column, value):
        return SimplePredicate(
            SimplePredicateType.ColumnToValue, column, value,
        )

    @v_args(inline=True)
    def and_predicate(self, predicate_i, predicate_j):
        return ComplexPredicate(
            ComplexPredicateType.AND, predicate_i, predicate_j,
        )

    @v_args(inline=True)
    def or_predicate(self, predicate_i, predicate_j):
        return ComplexPredicate(
            ComplexPredicateType.OR, predicate_i, predicate_j,
        )

    @v_args(inline=True)
    def not_predicate(self, predicate):
        return ComplexPredicate(
            ComplexPredicateType.NOT, predicate, None,
        )
